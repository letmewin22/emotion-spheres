...
void main() {
  ...
  vec2 uv = vUv;
    ...
    for (int i=0; i<ITERATIONS; i++) 
    {
        ...
        // col = mix(col, vec3(0.3* 3.-dist, 0.1* 2. -dist, 0.8* 2. -dist), mask);
        // col = mix(col, vec3(0.2+0.7*dist-0.1*offset-0.1, 0.5-0.5*offset-0.5*dist, 0.9), mask);
  ...
  vec4 finalTexture = vec4(col*1.5,1.0) + glassTexture*1.8;
  // vec4 finalTexture = glassTexture;
